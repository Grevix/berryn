...
## 2. Monorepo Architecture & Build Fix
- **Build Script Fix (`BRN-BUG-0007`)**: Updated root `package.json` `"build"` script from `"pnpm -r --filter \"./packages/*\" exec tsc --build"` to `"tsc --build"`. This ensures TypeScript compiles composite packages in exact topological dependency order using root `tsconfig.json` project references.
- **Typecheck Script**: `"typecheck": "tsc --build"`.
- **Vitest Alias Config**: [`vitest.config.ts`](file:///c:/Users/Aaryan%20Rawat/Downloads/Berryn/vitest.config.ts) resolves all `@berryn/*` workspace aliases to source files.
